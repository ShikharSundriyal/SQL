...
select  AVG(c.duration) , country.name from Calls c
join Person p on p.id = c.caller_id or c.callee_id=p.id
join Country country on split_part(P.phone_number,'-',1) = country.country_code
group by country.name having AVG(c.duration) > (select avg(duration) from calls)
;
